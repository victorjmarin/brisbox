...
Router.route('/edit-order/:ord', {
    template: "EditOrder",
    name: "edit-order",
    waitOn: function() {
        return Meteor.subscribe("oneOrder", Base64.decode(this.params.ord));
    },
    data: function (){
        var order = Orders.find({_id: Base64.decode(this.params.ord)}).fetch()[0];
        return order;
    }
})